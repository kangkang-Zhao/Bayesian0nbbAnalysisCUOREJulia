...
for datasets in datasets_group
    BI_priors[Symbol("BI_$(num)")] = BI_prior(datasets)
    global num += 1
end
...
likelihood = DensityInterface.logfuncdensity(
    function (param::NamedTuple)
        ...
        par_BI = []
        for datasets in datasets_group
            push!(par_BI, param[Symbol("BI_$(num)")])
            num += 1
            for ds in datasets
                par_bi = par_BI[num-1]
                par_bias = ForwardDiff.value(param[Symbol("bias_ds$(ds.ds)")])*1.0
                par_reso = ForwardDiff.value(param[Symbol("reso_ds$(ds.ds)")])*1.0
                par_effcut = ForwardDiff.value(param[Symbol("EffCut_ds$(ds.ds)")])*1.0
                ls = ds.lineshape
...
using Random  # 添加 Random 模块
using Random123
rng = Philox4x()
context = BATContext(rng = Philox4x(), ad = ADSelector(ForwardDiff))
...
@time @CPUtime samples = bat_sample(posterior, algorithm, context)
cpu_time = CPUtoc()
...
for key in keys(priors)
    ...
    savefig(p, "../crosscheck/$(par)_withSignal_Jan21.pdf")

end

import HDF5
bat_write("./SignalIncluded_Jan28.h5", samples.result)

...
# Now it's removed with BAT.jl, one can also find the substitute pakage EmpiricalDistributions.jl
module HistoPriorModule
export HistoPriorModule
using Distributions
using Statistics
using StatsBase
using BAT
...
export HistogramAsUvDistribution
...
function Base.rand(d::HistogramAsUvDistribution{T})::T where {T <: AbstractFloat}
    r::T = rand()
...
function Distributions.pdf(
    d::HistogramAsUvDistribution{T},
    x::Real
)::T where {T <: AbstractFloat}

    i = StatsBase.binindex(d.h, x)
    if i < 1 || i > length(d.h.weights)
        return zero(T)
    end

    edges = d.h.edges[1]

    @inbounds return d.h.weights[i]
end
...
# Base.eltype(d::HistogramAsUvDistribution{T}) where {T} = T
# Base.real_type(::Type{HistogramAsUvDistribution{T}}) where {T} = T
...
# my revision
# Implementionthe params method required by BAT.jl for a prior distribution
function Distributions.params(d::HistogramAsUvDistribution{T}) where T <: AbstractFloat
    return (edges=d.edges, weights=d.inv_weights, volumes=d.volumes)
end
